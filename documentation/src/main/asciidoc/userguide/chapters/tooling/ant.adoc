...
`<liquibase>`:: Generates a Liquibase changelog from schema differences.
...
[[tooling-ant-schema-liquibase]]
===== Liquibase Changelog Generation

The `<liquibase>` exporter compares the entity model against the current database schema and
generates a Liquibase XML changelog containing the differences.

[source,xml]
----
<hibernatetool destdir="${build.dir}/generated-resources">
    <jpaconfiguration persistenceunit="default"/>
    <liquibase/>
</hibernatetool>
----

The `<liquibase>` element supports the following attributes:
...
`destdir`:: Override the output directory for this exporter.
