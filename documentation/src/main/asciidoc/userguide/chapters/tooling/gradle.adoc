...
<<tooling-gradle-reveng,reverse engineering>>, and <<tooling-gradle-schema,schema management>> capabilities into Gradle builds.
...
DAOs, configuration files, DDL scripts, and Flyway migrations from an existing database.
...
[[tooling-gradle-schema]]
==== Schema Management

The Hibernate ORM Gradle plugin provides a `generateFlyway` task that generates Flyway versioned
migration files by comparing JPA entity classes against the existing database using `SchemaUpdate`.
Unlike the reverse engineering tasks which read _from_ the database, the Flyway task works in the
opposite direction: entity classes define the target schema, and the migration captures the diff.

The task automatically depends on `classes` so that entity classes are compiled first.


[[tooling-gradle-schema-configuration]]
===== Configuration

Flyway migration settings are configured through the `reveng` block inside the `hibernate` extension:
...
hibernate {
    reveng {
        ...
        migrationVersion = '2'
        migrationDescription = 'add email column'
    }
}
----

`persistenceUnit`:: The name of the JPA persistence unit to use for entity discovery. The `META-INF/persistence.xml` must be in the project's main resource set. Default: `default`.
`migrationVersion`:: The version number for the generated migration file. Default: `null` (uses the exporter default of `1`).
`migrationDescription`:: A human-readable description for the migration. Spaces are replaced with underscores. Default: `null` (uses the exporter default of `hibernate_schema_update`).

The generated file follows the Flyway naming convention: `V{version}__{description}.sql`.


[[tooling-gradle-schema-example]]
===== Example

Create a `META-INF/persistence.xml` in `src/main/resources/` that lists your entity classes explicitly:
...
NOTE: Gradle places resources in `build/resources/main/` and compiled classes in
`build/classes/java/main/` — separate directories. Hibernate scans the persistence unit root
(where `META-INF/persistence.xml` lives), which is the resources directory, so entity classes
in the classes directory are not found automatically. Listing them with `<class>` elements
is the simplest solution. Alternatively, you can copy `persistence.xml` into the classes
directory so that everything shares the same root:

[source,gradle]
----
tasks.named('generateFlyway') {
    doFirst {
        copy {
            from 'src/main/resources/META-INF/persistence.xml'
            into "${sourceSets.main.java.classesDirectory.get()}/META-INF"
        }
    }
}
----

Configure the Flyway migration settings in `build.gradle`:
