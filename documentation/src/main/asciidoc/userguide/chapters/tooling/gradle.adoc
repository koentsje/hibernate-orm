...
<<tooling-gradle-reveng,reverse engineering>>,
and <<tooling-gradle-liquibase,Liquibase changelog generation>> capabilities into Gradle builds.
...
[[tooling-gradle-liquibase]]
==== Liquibase Changelog Generation

The Hibernate ORM Gradle plugin provides a `generateLiquibase` task that compares your JPA
entity model against the current database schema and generates a Liquibase XML changelog
containing the differences.  The task depends on `classes`, so entity classes are compiled
first.


[[tooling-gradle-liquibase-configuration]]
===== Task Configuration

The task exposes the following properties that can be set directly:

[source,gradle]
----
tasks.named('generateLiquibase') {
    persistenceUnit = 'default'
    outputFileName = 'changelog.xml'
    changelogFormat = 'xml'
    changesetAuthor = 'hibernate'
    haltOnError = false
}
----

`persistenceUnit`:: The name of the JPA persistence unit to use. Default: `default`.
`outputDirectory`:: The directory where the changelog file is written. Default: `build/generated-resources`.
...
[[tooling-gradle-liquibase-example]]
===== Example
...
[source,bash]
----
gradle generateLiquibase
----

The task reads the entity model from the specified persistence unit, connects to the database
