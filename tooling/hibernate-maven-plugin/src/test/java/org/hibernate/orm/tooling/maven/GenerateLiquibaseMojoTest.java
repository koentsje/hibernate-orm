...
public class GenerateLiquibaseMojoTest {

	@TempDir
	private File tempDir;

	private File outputDirectory;
	private GenerateLiquibaseMojo mojo;
	...
	public void testGenerateChangelog() throws Exception {
		mojo.executeExporter( createJpaDescriptor() );
	...
	public void testCustomAuthor() throws Exception {
		setField( mojo, "changesetAuthor", "koen" );
		mojo.executeExporter( createJpaDescriptor() );
		File changelog = new File( outputDirectory, "changelog.xml" );
		String xml = Files.readString( changelog.toPath() );
	...
	public void testCustomOutputFileName() throws Exception {
		setField( mojo, "outputFileName", "migrations.xml" );
		mojo.executeExporter( createJpaDescriptor() );
	...
	private JpaMetadataDescriptor createJpaDescriptor() throws Exception {
		...
		try {
			...
			return new JpaMetadataDescriptor( "liquibase-mojo-test", props );
		}
		finally {
	...
	private void writePersistenceXml() throws Exception {
		...
		Files.writeString( new File( metaInf, "persistence.xml" ).toPath(),
				...
				<persistence xmlns="https://jakarta.ee/xml/ns/persistence" version="3.0">
					<persistence-unit name="liquibase-mojo-test">
						<class>org.hibernate.orm.tooling.maven.GenerateLiquibaseMojoTest$Person</class>
	...
	private void createMojo() throws Exception {
		mojo = new GenerateLiquibaseMojo();
		setField( mojo, "outputDirectory", outputDirectory );
		setField( mojo, "outputFileName", "changelog.xml" );
		setField( mojo, "changelogFormat", "xml" );
		setField( mojo, "changesetAuthor", "hibernate" );
		setField( mojo, "haltOnError", true );
	}
